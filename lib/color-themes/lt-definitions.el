...
(defconst lt-description
  "A Light Table inspired color theme based on Solarized's definitions.")

(defcustom lt-degrade nil
  "For test purposes only; when in GUI mode, forces Lt to use the 256
degraded color mode to test the approximate color values for accuracy."
  :type 'boolean
  :group 'lt)

(defcustom lt-diff-mode 'normal
...
(defcustom lt-bold t
  "Stops Lt from displaying bold when nil."
  :type 'boolean
  :group 'lt)

(defcustom lt-underline t
  "Stops Lt from displaying underlines when nil."
  :type 'boolean
  :group 'lt)

(defcustom lt-italic t
  "Stops Lt from displaying italics when nil."
  :type 'boolean
  :group 'lt)

(defcustom lt-contrast 'normal
  "Stick with normal! It's been carefully tested. Setting this option to high or
low does use the same Lt palette but simply shifts some values up or
down in order to expand or compress the tonal range displayed."
  :type 'symbol
  :options '(high normal low)
  :group 'lt)

(defcustom lt-broken-srgb (if (and (eq system-type 'darwin)
                                          (eq window-system 'ns))
                                     t
                                   nil)
  "Emacs bug #8402 results in incorrect color handling on Macs. If this is t
\(the default on Macs), Lt works around it with alternative colors.
However, these colors are not totally portable, so you may be able to edit
the \"Gen RGB\" column in lt-definitions.el to improve them further."
  :type 'boolean
  :group 'lt)
...
(defvar lt-colors           ; ANSI(Lt terminal)
  ...
  '((base03  "#002b36" "#202020" "#1c1c1c" "brightblack"   "black")
    ...
    (yellow  "#b58900" "#aaeecc" "#af8700" "yellow"        "yellow")
    (orange  "#cb4b16" "#ff8888" "#d75f00" "brightred"     "red")
    (red     "#dc322f" "#ff3333" "#d70000" "red"           "red")
    (magenta "#d33682" "#FF1F69" "#af005f" "magenta"       "magenta")
    (violet  "#6c71c4" "#ccaaff" "#5f5faf" "brightmagenta" "magenta")
    (blue    "#268bd2" "#aaccff" "#0087ff" "blue"          "blue")
    (cyan    "#2aa198" "#aadddd" "#00afaf" "cyan"          "cyan")
    (white   "#ffffff" "#ffffff" "#ffffff" "white"          "white")
    (green   "#859900" "#aaffaa" "#5f8700" "green"         "green"))
  "This is a table of all the colors used by the Lt color theme. Each
   column is a different set, one of which will be chosen based on term
   capabilities, etc.")

(defun lt-color-definitions (mode)
  (flet ((find-color (name)
           (let* ((index (if window-system
                             (if lt-degrade
                                 3
                               (if lt-broken-srgb 2 1))
                           (case (display-color-cells)
                             (16 4)
                             (8  5)
                             (otherwise 3)))))
             (nth index (assoc name lt-colors)))))
    (let ((base03      (find-color 'base03))
          ...
          (bold        (if lt-bold 'bold 'normal))
          (bright-bold (if lt-bold 'normal 'bold))
          (underline   (if lt-underline t nil))
          (opt-under   nil)
          (italic      (if lt-italic 'italic 'normal)))
      (when (eq 'light mode)
...
(defmacro create-lt-theme (mode)
  (let* ((theme-name (intern (concat "lt-" (symbol-name mode))))
         (defs (lt-color-definitions mode))
         (theme-vars (mapcar (lambda (def) (list (car def) (cdr def)))
                             (second defs)))
         (theme-faces (first defs)))
    `(progn
       (deftheme ,theme-name ,lt-description)
